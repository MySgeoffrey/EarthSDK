...
{
	osgViewer::Viewer* pViewer = dynamic_cast<osgViewer::Viewer*>(view);
	if (pViewer != NULL)
	{
		bool hasGetCoor = false;
		// 获取当前点
		osgUtil::LineSegmentIntersector::Intersections intersection;
		pViewer->computeIntersections(x,y, intersection);
		...
		while (iter != intersection.end())
		{
			...
			{
				out_coords = iter->getWorldIntersectPoint();
				hasGetCoor = true;
				break;
			}
			++iter;
		...
		if (!hasGetCoor)
		{
			const osg::Camera* camera = view->getCamera();//view2->getCameraContainingPosition(x, y, local_x, local_y);
			if (!camera) 
			{
				osg::Matrixd matrix;
	...
	virtual void set(const GeoPoint& coords, osg::View* view, MapNode* mapNode)
	{
		if (this->mpOwner)
		{
			...
			if (this->mpOwner->getStateInfoVisible())
			{
				...
				std::string g_longtitudeStr = ::CLanguageConfig::getValue(CLanguageConfig::StateTextType_Longitude);
				std::string g_lantitudeStr = ::CLanguageConfig::getValue(CLanguageConfig::StateTextType_Lantitude);
				std::string g_terrainAltitudeStr = ::CLanguageConfig::getValue(CLanguageConfig::StateTextType_TerrainAltitude);
				std::string g_viewAltitudeStr = ::CLanguageConfig::getValue(CLanguageConfig::StateTextType_ViewAltitude);

				CGlobeSetting::instance()->gb2312ToUtf8(g_longtitudeStr, lonStr);
				CGlobeSetting::instance()->gb2312ToUtf8(g_lantitudeStr, latStr);
				CGlobeSetting::instance()->gb2312ToUtf8(g_terrainAltitudeStr, altStr);
				CGlobeSetting::instance()->gb2312ToUtf8(g_viewAltitudeStr, cameraStr);
...
#else
				std::string lonStrEx = this->mpOwner->mLongitudeVisible ? (lonStr + do_fraction(coords.x(), this->mpOwner->getPrecision()) + "  " ) : "";
				std::string latStrEx = this->mpOwner->mLantitudeVisible ? (latStr + do_fraction(coords.y(), this->mpOwner->getPrecision()) + "  " ) : "";
				std::string demStrEx = this->mpOwner->mTerrainAltitudeVisible ? (altStr + do_fraction(demAlt, this->mpOwner->getPrecision()) + "  " ) : "";
				std::string viewStrEx = this->mpOwner->mViewAltitudeVisible ? (cameraStr + do_fraction(distanceToGround, this->mpOwner->getPrecision()) + "  " ) : "";
...
public:
	...
	virtual void operator()(osg::Node* node, osg::NodeVisitor* nv)
	{
		if (this->mpOwner)
		{
			...
			{
				osg::Vec3d startWorldPoint,endWorldPoint;
				double x = this->mpOwner->mpRefGlobeWidgetEx->width() / 2;
				double y = this->mpOwner->mpRefGlobeWidgetEx->height() / 2;
				double deltaPixel = 100;
				getWorldCoordsUnderMouse((osgViewer::Viewer*)this->mpOwner->mpRefGlobeWidgetEx->getViewer(),x - deltaPixel / 2,y,startWorldPoint);
				getWorldCoordsUnderMouse((osgViewer::Viewer*)this->mpOwner->mpRefGlobeWidgetEx->getViewer(),x + deltaPixel / 2,y,endWorldPoint);
				double realDis = (endWorldPoint - startWorldPoint).length();
				double mapDis = 100 * 0.3 /1000.0;
				double scale = mapDis / realDis;
				...
				else if(imageRealDis >= 1)
				{
					sprintf(imageRealDisStr, "%.0fm", imageRealDis);
				}
				else 
				{
					sprintf(imageRealDisStr, "%.3fm", imageRealDis);
				}
	...
	mEagleVisible(false), mEagleImageControl(NULL), mEaglePositionControl(NULL)
{
	...
	this->mPrecision = 6;
	this->mLongitudeVisible = true;
	this->mLantitudeVisible = true;
	this->mTerrainAltitudeVisible = true;
	this->mViewAltitudeVisible = true;
}
...
void CGlobeNavigation::inital(osg::Camera*& pScreenCamera)
{
	this->mLabelCamera = pScreenCamera;
	...
	//初始化创建级别放大键
	std::string leveZoomInControlTexturePath = osgDB::getFilePath(longPath) + "/systemImages/navigation/zoomIn.png";
	mZoomInLevelControl = new ImageControl();
	osg::Image* pLeveZoomInControlTexture = osgDB::readImageFile(leveZoomInControlTexturePath);
	mZoomInLevelControl->setImage(pLeveZoomInControlTexture);
	mZoomInLevelControl->setMargin(10);
	mZoomInLevelControl->setVisible(false);

	//初始化创建级别缩小键
	std::string leveZoomOutControlTexturePath = osgDB::getFilePath(longPath) + "/systemImages/navigation/zoomOut.png";
	mZoomOutLevelControl = new ImageControl();
	osg::Image* pLeveZoomOutControlTexture = osgDB::readImageFile(leveZoomOutControlTexturePath);
	mZoomOutLevelControl->setImage(pLeveZoomOutControlTexture);
	mZoomOutLevelControl->setMargin(10);
	mZoomOutLevelControl->setVisible(false);
	...
	canvas->addControl(mZoomInLevelControl);
	canvas->addControl(mZoomOutLevelControl);
...
void CGlobeNavigation::resize(const int& width,const int& height)
{
	if (mHasInitialised)
	{
		if (this->getNavigationVisible())
		{
			...
			this->mZoomInLevelControl->setPosition(this->mX + 29, this->mY + 65);
			this->mZoomOutLevelControl->setPosition(this->mX + 29, this->mY + 228);
...
bool CGlobeNavigation::handle(const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& aa)
{
	...
	else if (ea.getEventType() == osgGA::GUIEventAdapter::RELEASE && ea.getButton() == mMouseButton)
	{
		...
		if (earthManip)
		{
			...
			if (this->contains(this->mZoomInLevelControl, ea.getX(), ea.getY()))
			{
				earthManip->zoom(0, -0.5);
				aa.requestRedraw();
				ea.setHandled(true);
			}
			else if (this->contains(this->mZoomOutLevelControl, ea.getX(), ea.getY()))
			{
				earthManip->zoom(0, 0.5);
				aa.requestRedraw();
				ea.setHandled(true);
	...
	else if (ea.getEventType() == osgGA::GUIEventAdapter::MOVE)
	{
		...
		this->mZoomInLevelControl->setVisible(this->contains(this->mZoomInLevelControl, ea.getX(), ea.getY()));
		this->mZoomOutLevelControl->setVisible(this->contains(this->mZoomOutLevelControl, ea.getX(), ea.getY()));
	}
...
int CGlobeNavigation::getPrecision()
{
	return this->mPrecision;
}

void CGlobeNavigation::setPrecision(const int& precision)
{
