...
osg::Vec3d g_intersectWorldPos = osg::Vec3d(0,0,0);
...
namespace
{
    ...
    {
        ManipTerrainCallback(VREarthManipulator* manip) : _manip(manip) { }
        void onTileAdded(const TileKey& key, osg::Node* tile, TerrainCallbackContext& context)
        {
            osg::ref_ptr<VREarthManipulator> safe;
            if ( _manip.lock(safe) )
            {
                safe->handleTileAdded(key, tile, context);
            }
        }
        osg::observer_ptr<VREarthManipulator> _manip;
    };
}



VREarthManipulator::Action::Action( ActionType type, const ActionOptions& options ) :
_type( type ),
_options( options )
{
    init();
...
VREarthManipulator::Action::Action( ActionType type ) :
_type( type )
{
    init();
}
...
void
VREarthManipulator::Action::init()
{
    _dir =
...
VREarthManipulator::Action::Action( const Action& rhs ) :
_type( rhs._type ),
_dir( rhs._dir ),
...
bool
VREarthManipulator::Action::getBoolOption( int option, bool defaultValue ) const
{
...
int
VREarthManipulator::Action::getIntOption( int option, int defaultValue ) const
{
    for(ActionOptions::const_iterator i = _options.begin(); i != _options.end(); i++ ) {
        if ( i->option() == option )
            return i->intValue();
...
double
VREarthManipulator::Action::getDoubleOption( int option, double defaultValue ) const
{
    for(ActionOptions::const_iterator i = _options.begin(); i != _options.end(); i++ ) {
        if ( i->option() == option )
...
VREarthManipulator::Action VREarthManipulator::NullAction( VREarthManipulator::ACTION_NULL );
...
VREarthManipulator::Settings::Settings() :
osg::Referenced                 (),
Revisioned                      (),
...
VREarthManipulator::Settings::Settings( const VREarthManipulator::Settings& rhs ) :
osg::Referenced( rhs ),
Revisioned     ( rhs ),
...
void
VREarthManipulator::Settings::apply(osg::ArgumentParser& args)
{
    bool   boolval;
    double doubleval;
...
void
VREarthManipulator::Settings::expandSpec( const InputSpec& input, InputSpecs& output ) const
{
...
void
VREarthManipulator::Settings::bind( const InputSpec& spec, const Action& action )
{
    InputSpecs specs;
...
void
VREarthManipulator::Settings::bindMouse(ActionType actionType,
...
void
VREarthManipulator::Settings::bindMouseClick(ActionType action,
                                           int button_mask, int modkey_mask,
                                           const ActionOptions& options)
{
    bind(
...
void
VREarthManipulator::Settings::bindMouseDoubleClick(ActionType action,
                                                 int button_mask, int modkey_mask,
                                                 const ActionOptions& options)
{
    bind(
...
void
VREarthManipulator::Settings::bindKey(ActionType action,
...
void
VREarthManipulator::Settings::bindScroll(ActionType action, int scrolling_motion,
...
void
VREarthManipulator::Settings::bindPinch(ActionType action, const ActionOptions& options)
{
    bind(
        InputSpec( VREarthManipulator::EVENT_MULTI_PINCH, 0, 0 ),
        Action( action, options ) );
}

void
VREarthManipulator::Settings::bindTwist(ActionType action, const ActionOptions& options)
{
    bind(
         InputSpec( VREarthManipulator::EVENT_MULTI_TWIST, 0, 0 ),
         Action( action, options ) );
}

void
VREarthManipulator::Settings::bindMultiDrag(ActionType action, const ActionOptions& options)
{
    bind(
        InputSpec( VREarthManipulator::EVENT_MULTI_DRAG, 0, 0 ),
        Action( action, options ) );
}

const VREarthManipulator::Action&
VREarthManipulator::Settings::getAction(int event_type, int input_mask, int modkey_mask) const
{
...
void
VREarthManipulator::Settings::setMinMaxPitch( double min_pitch, double max_pitch )
{
...
void
VREarthManipulator::Settings::setMaxOffset(double max_x_offset, double max_y_offset)
{
...
void
VREarthManipulator::Settings::setMinMaxDistance( double min_distance, double max_distance)
{
...
void
VREarthManipulator::Settings::setArcViewpointTransitions( bool value )
{
...
void
VREarthManipulator::Settings::setAutoViewpointDurationEnabled( bool value )
{
...
void
VREarthManipulator::Settings::setAutoViewpointDurationLimits( double minSeconds, double maxSeconds )
{
...
VREarthManipulator::VREarthManipulator() :
...
VREarthManipulator::VREarthManipulator(osg::ArgumentParser& args) :
osgGA::CameraManipulator()
{
    ctor_init();
    reinitialize();
...
VREarthManipulator::VREarthManipulator( const VREarthManipulator& rhs ) :
...
void
VREarthManipulator::configureDefaultSettings()
{
...
void
VREarthManipulator::applySettings( Settings* settings )
{
    if ( settings )
    {
        _settings = settings;
...
VREarthManipulator::Settings*
VREarthManipulator::getSettings() const
{
    return _settings.get();
}

void
VREarthManipulator::reinitialize()
{
    _distance = 1.0;
    _viewOffset.set(0,0);
...
bool
VREarthManipulator::established()
{
...
void
VREarthManipulator::handleTileAdded(const TileKey& key, osg::Node* tile, TerrainCallbackContext& context)
{
...
bool
VREarthManipulator::createLocalCoordFrame( const osg::Vec3d& worldPos, osg::CoordinateFrame& out_frame ) const
{
    if ( _srs.valid() )
    {
        osg::Vec3d mapPos;
...
void
VREarthManipulator::setCenter( const osg::Vec3d& worldPos )
{
    _center = worldPos;
...
void
VREarthManipulator::setNode(osg::Node* node)
{
...
osg::Node*
VREarthManipulator::getNode()
{
    return _node.get();
}

osg::Matrixd
VREarthManipulator::getRotation(const osg::Vec3d& point) const
{
...
osg::Quat
VREarthManipulator::computeCenterRotation(const osg::Vec3d& point) const
{
...
Viewpoint
VREarthManipulator::getViewpoint() const
{
    Viewpoint vp;
...
void
VREarthManipulator::breakTether()
{
...
void
VREarthManipulator::setViewpoint(const Viewpoint& vp, double duration_seconds)
{
...
double
VREarthManipulator::setViewpointFrame(double time_s)
{
...
void
VREarthManipulator::setLookAt(const osg::Vec3d& center,
                            double            azim,
                            double            pitch,
                            double            range,
...
void
VREarthManipulator::resetLookAt()
{
    double pitch;
...
bool
VREarthManipulator::isSettingViewpoint() const
{
    return _setVP0.isSet() && _setVP1.isSet();
}

void
VREarthManipulator::cancelViewpointTransition()
{
...
void
VREarthManipulator::clearViewpoint()
{
...
bool
VREarthManipulator::isTethering() const
{
...
void
VREarthManipulator::setTetherNode(osg::Node* node, double duration_s)
{
...
void
VREarthManipulator::setTetherNode(osg::Node* node,
                                double     duration_s,
...
osg::Node*
VREarthManipulator::getTetherNode() const
{
    if ( !isTethering() )
        return 0L;
...
void VREarthManipulator::collisionDetect()
{
    if (!getSettings()->getTerrainAvoidanceEnabled() ||
        !_srs.valid() )
    {
...
bool
VREarthManipulator::intersect(const osg::Vec3d& start, const osg::Vec3d& end, osg::Vec3d& intersection, osg::Vec3d& normal) const
{
    osg::ref_ptr<MapNode> mapNode;
...
bool
VREarthManipulator::intersectLookVector(osg::Vec3d& out_eye,
...
void
VREarthManipulator::home(double unused)
{
    handleAction( ACTION_HOME, 0, 0, 0 );
}

void
VREarthManipulator::home(const osgGA::GUIEventAdapter& ,osgGA::GUIActionAdapter& us)
{
    home( 0.0 );
    us.requestRedraw();
}
...
void
VREarthManipulator::computeHomePosition()
{
    if( getNode() )
    {
...
void
VREarthManipulator::init(const osgGA::GUIEventAdapter&, osgGA::GUIActionAdapter& )
{
    flushMouseEventStack();
}
...
void
VREarthManipulator::getUsage(osg::ApplicationUsage& usage) const
{
}

void
VREarthManipulator::resetMouse( osgGA::GUIActionAdapter& aa, bool flushEventStack )
{
    if (flushEventStack)
      flushMouseEventStack();
...
void
VREarthManipulator::updateProjection(osg::Camera* eventCamera)
{
...
bool
VREarthManipulator::handle(const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& aa)
{
    bool handled = false;
...
#if 0
				if ( screenToWorld( ea.getX(), ea.getY(), view, intersectWorldPos ))
				{
					g_intersectWorldPos = intersectWorldPos;
				}
#endif
                break;
...
void
VREarthManipulator::updateTether()
{
    double t = 1.0;
...
Viewpoint
VREarthManipulator::getTetherNodeViewpoint() const
{
...
bool
VREarthManipulator::serviceTask()
{
    if ( _task.valid() && _task->_type != TASK_NONE )
    {
...
bool
VREarthManipulator::isMouseMoving()
{
...
bool
VREarthManipulator::isMouseClick( const osgGA::GUIEventAdapter* mouse_up_event ) const
{
...
void
VREarthManipulator::flushMouseEventStack()
{
    _ga_t1 = NULL;
    _ga_t0 = NULL;
    //_touchPointQueue.clear();
...
void
VREarthManipulator::addMouseEvent(const osgGA::GUIEventAdapter& ea)
{
    _ga_t1 = _ga_t0;
    _ga_t0 = &ea;
    //_touchPointQueue.clear();
...
void
VREarthManipulator::addTouchEvents(const osgGA::GUIEventAdapter& ea)
{
    _ga_t1 = _ga_t0;
    _ga_t0 = &ea;
...
bool
VREarthManipulator::parseTouchEvents( TouchEvents& output )
{
...
void
VREarthManipulator::setByMatrix(const osg::Matrixd& matrix)
{
    if (!established())
        return;
...
osg::Matrixd
VREarthManipulator::getMatrix() const
{
...
osg::Matrixd
VREarthManipulator::getInverseMatrix() const
{
...
void
VREarthManipulator::updateCamera(osg::Camera& camera)
{
...
void
VREarthManipulator::setByLookAt(const osg::Vec3d& eye,const osg::Vec3d& center,const osg::Vec3d& up)
{
    osg::ref_ptr<osg::Node> safeNode;
    if ( _node.lock(safeNode) )
    {
...
void
VREarthManipulator::setByLookAtRaw(const osg::Vec3d& eye,const osg::Vec3d& center,const osg::Vec3d& up)
{
    // compute rotation matrix
    osg::Vec3d lv(center-eye);
    setDistance( lv.length() );
...
bool
VREarthManipulator::recalculateCenterFromLookVector()
{
...
void
VREarthManipulator::recalculateCenter( const osg::CoordinateFrame& frame )
{
    osg::ref_ptr<osg::Node> node;
    if ( _node.lock(node) )
    {
...
void
VREarthManipulator::pan( double dx, double dy )
{
    if ( !isTethering() )
    {
...
void
VREarthManipulator::rotate( double dx, double dy )
{
	...
	if (g_intersectWorldPos != osg::Vec3d(0,0,0))
		_center = g_intersectWorldPos;

	osg::Vec3d rotatePosition = _center;
	osg::Vec3d cameraPosition = this->getMatrix().getTrans();
	osg::Vec3d cameraDir = cameraPosition - rotatePosition;
	double dis = cameraDir.normalize();

	osg::Quat rotate_azim ;
	osg::Vec3d axis = rotatePosition;
	axis.normalize();
	osg::Vec3d localUp(0.0f,0.0f,1.0f);
	axis = localUp;
	rotate_azim.makeRotate(-dx,axis);

	osg::Quat rotate_elevation ;
	rotate_elevation.makeRotate(dy,sideVector);
	osg::Quat q = rotate_azim * rotate_elevation;

	osg::Vec3d relativeDir = q * cameraDir;
	relativeDir.normalize();
	osg::Vec3d curCameraPos = rotatePosition + relativeDir * dis;

	_center = rotatePosition;
    _rotation = _rotation * rotate_elevation * rotate_azim;
#endif
    collisionDetect();
}
...
void
VREarthManipulator::zoom( double dx, double dy )
{
...
bool
VREarthManipulator::screenToWorld(float x, float y, osg::View* theView, osg::Vec3d& out_coords) const
{
...
void
VREarthManipulator::setDistance( double distance )
{
    _distance = osg::clampBetween( distance, _settings->getMinDistance(), _settings->getMaxDistance() );
}

void
VREarthManipulator::dumpActionInfo( const VREarthManipulator::Action& action, osg::NotifySeverity level ) const
{
...
void
VREarthManipulator::handleMovementAction( const ActionType& type, double dx, double dy, osg::View* view )
{
    switch( type )
    {
    case ACTION_PAN:
...
bool
VREarthManipulator::handlePointAction( const Action& action, float mx, float my, osg::View* view )
{
...
void
VREarthManipulator::handleContinuousAction( const Action& action, osg::View* view )
{
...
void
VREarthManipulator::applyOptionsToDeltas( const Action& action, double& dx, double& dy )
{
...
bool
VREarthManipulator::handleMouseAction( const Action& action, osg::View* view )
{
...
bool
VREarthManipulator::handleMouseClickAction( const Action& action )
{
    //TODO.
    return false;
}
...
bool
VREarthManipulator::handleKeyboardAction( const Action& action, double duration )
{
    double dx = 0, dy = 0;
...
bool
VREarthManipulator::handleScrollAction( const Action& action, double duration )
{
...
bool
VREarthManipulator::handleAction( const Action& action, double dx, double dy, double duration )
{
    bool handled = true;
...
void
VREarthManipulator::recalculateRoll()
{
    osg::Matrixd rotation_matrix;
...
void
VREarthManipulator::getCompositeEulerAngles( double* out_azim, double* out_pitch ) const
{
...
void
VREarthManipulator::getEulerAngles(const osg::Quat& q, double* out_azim, double* out_pitch) const
{
    osg::Matrix m( q );
...
osg::Quat
VREarthManipulator::getQuaternion(double azim, double pitch) const
{
...
void
VREarthManipulator::collapseTetherRotationIntoRotation()
{
...
void
VREarthManipulator::setHomeViewpoint( const Viewpoint& vp, double duration_s )
{
    _homeViewpoint = vp;
...
// rotation. VREarthManipulator represents the placement matrix as the
...
void
VREarthManipulator::drag(double dx, double dy, osg::View* theView)
{
    osgViewer::View* view = dynamic_cast<osgViewer::View*>(theView);
    if ( !view )
        return;
