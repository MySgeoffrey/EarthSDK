...
STRUCT__IFC__ITEM *CreateIfcItem(int ifcModel, STRUCT__IFC__ITEM * parent, int ifcID, wchar_t * ifcType, bool hide, wchar_t * globalId, wchar_t * name, wchar_t * description)
{
	...
	ifcItem->description = __copy__(description);

	ifcItem->propertySets = 0;
	ifcItem->propertySetsAvailableButNotLoadedYet = false;

	ifcItem->verticesForFaces = 0;
	ifcItem->verticesForWireFrame = 0;
	...
	ifcItem->vertexOffsetForFaces = 0;
	ifcItem->indexOffsetForFaces = 0;

	ifcItem->noVerticesForWireFrame = 0;
	ifcItem->noPrimitivesForWireFrame = 0;
	ifcItem->verticesForWireFrame = 0;
	ifcItem->indicesForWireFrame = 0;
	ifcItem->indicesForWireFrameLineParts = 0;
	ifcItem->vertexOffsetForWireFrame = 0;
	ifcItem->indexOffsetForWireFrame = 0;
